...
\end{theorem}
\begin{proof}
Let \(w\in[0,1]\) be the probability of expert one. By affine randomized-selection risk, fixed-expert excess risks are \(r_1(w)=(1-w)\delta_1\) and \(r_2(w)=w\delta_2\). The first decreases and the second increases. Their maximum is minimized at their unique intersection; otherwise moving toward the intersection reduces the larger term. Solving gives \(w^\star=\delta_1/(\delta_1+\delta_2)\), and substitution gives the stated positive value. Conditioning on the contract chooses the zero-risk expert in each environment. This is not V5 matched-action regret.
\end{proof}
