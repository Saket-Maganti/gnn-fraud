...
\end{aligned}
\]
All source group risks, source training surrogates, and CVaR terms
are aggregated from example predictions, labels, group indices, expert
predictions, and availability masks, preserving gradients through the
contract encoder and router. These fitting terms are distinct from the frozen
V5 target estimand, whose row-wise feasible hindsight oracle includes the same
abstention action available to the method. Source-contract abstention capacities govern training and
balanced source validation. The selected threshold and decision are frozen
before target scoring; target capacity can only constrain that label-free
decision. Source-contract budgets produce separate group-local top-$K$
surrogates, while unconstrained groups receive no artificial $K$. We do not
claim exact optimisation of discontinuous Recall@$K$. Stability covers
contract perturbations, score noise, and availability masks.
