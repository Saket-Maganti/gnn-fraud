...
\end{aligned}
\]
Task and CVaR surrogates use source labels; deployment never does. They are distinct from the frozen V5 target metric, whose row-wise feasible hindsight oracle includes the same abstention action available to the method. Budget surrogates are group-local and do not claim exact optimization of discontinuous Recall@\(K\). Coefficients and source-validation thresholds are frozen before any target result is unlocked.
