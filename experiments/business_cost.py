...
def build_model_table(results_dir: str) -> pd.DataFrame:
    """
    Combine metrics.csv, seed_results.csv, and temporal_deep.csv into a
    unified table with model, precision, recall, f1, source columns.

    Falls back gracefully when any source is missing.
    """
    _COLS = ["model", "variant", "f1", "precision", "recall", "source"]
    rows = []
    ...
    if os.path.exists(metrics_path):
        ...
        if {"model", "f1", "precision", "recall"}.issubset(df.columns):
    ...
    # From seed_results.csv (mean values across N seeds)
    seed_path = os.path.join(results_dir, "seed_results.csv")
    if os.path.exists(seed_path):
        df = pd.read_csv(seed_path)
        if {"model", "strategy", "f1_mean", "precision_mean", "recall_mean"}.issubset(df.columns):
            for _, r in df.iterrows():
                rows.append({
    ...
    # From temporal_deep.csv — aggregate per-timestep stats to model-level means.
    # This file is produced by experiments/temporal_deep.py (Part 1 of run_upgrades.sh)
    # and is always available after the standard run.
    temporal_path = os.path.join(results_dir, "temporal_deep.csv")
    if os.path.exists(temporal_path):
        df = pd.read_csv(temporal_path)
        if {"model", "f1", "precision", "recall"}.issubset(df.columns):
            agg = (
                df.groupby("model")[["f1", "precision", "recall"]]
                .mean()
                .reset_index()
            )
            for _, r in agg.iterrows():
                rows.append({
                    "model":     r["model"],
                    "variant":   "temporal_mean",
                    "f1":        r["f1"],
                    "precision": r["precision"],
                    "recall":    r["recall"],
                    "source":    "temporal_deep.csv",
                })
...
REPRESENTATIVE_MODELS = [
    ...
    # Primary sources: metrics.csv / seed_results.csv (from full baseline run)
    ...
    # Fallback: temporal_deep.csv (always generated by Parts 0+1 of run_upgrades.sh)
    ("SAGE-weighted (temporal avg)", "sage_weighted",   "temporal_mean","temporal_deep.csv"),
    ("SAGE-graph_aug (temporal avg)","sage_graph_aug",  "temporal_mean","temporal_deep.csv"),
    ("GAT-weighted (temporal avg)",  "gat_weighted",    "temporal_mean","temporal_deep.csv"),
    ("GAT-graph_aug (temporal avg)", "gat_graph_aug",   "temporal_mean","temporal_deep.csv"),
]
...
def main(args):
    ...
    if df_cost.empty:
        print("ERROR: No model data found. Tried:")
        print("       results/metrics.csv      (run experiments/baselines.py)")
        print("       results/seed_results.csv (run experiments/run_seeds.py)")
        print("       results/temporal_deep.csv (run_upgrades.sh part 0+1)")
        sys.exit(1)
